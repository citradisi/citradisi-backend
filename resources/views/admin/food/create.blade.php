            ...
            <form action="{{ route('food.create') }}" method="POST" enctype="multipart/form-data">
              ...
              <div class="card-body">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="food_name" id="food_name" placeholder="Food Name" value="{{ old('food_name') }}">
                  @error('food_name') <span class="text-danger"> {{ $message }}</span> @enderror
                </div>

                <div class="form-group">
                  <label for="food_material_id">Food Material (Indonesia)</label>
                  <textarea name="food_material_id" id="food_material_id" cols="30" rows="10" class="form-control" placeholder="Food Description">{{ old('food_material_id') }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea name="food_material_en" id="food_material_en" cols="30" rows="10" class="form-control" placeholder="Food Description">{{ old('food_material_en') }}</textarea>
                    @error('food_material_en') <span class="text-danger"> {{ $message }}</span> @enderror
                  </div>

                <div class="form-group">
                  <label for="food_make_id">Food Make (Indonesia)</label>
                  <textarea name="food_make_id" id="food_make_id" cols="30" rows="10" class="form-control" placeholder="Food Description">{{ old('food_make_id') }}</textarea>
                  @error('food_make_id') <span class="text-danger"> {{ $message }}</span> @enderror
                </div>

                <div class="form-group">
                  <label for="food_make_en">Food Make (English)</label>
                  <textarea name="food_make_en" id="food_make_en" cols="30" rows="10" class="form-control" placeholder="Food Description">{{ old('food_make_en') }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <div class="form-group">
                        <img id="showImage" class="img-thumbnail mt-2" src="{{ asset('backend/dist/img/no-image.jpg') }}" alt="Image Show">
                    </div>
                <!-- end row -->
